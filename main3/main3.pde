...
SoundFile gameloose_music,attack_music,jump_music,women_music;
...
PImage background;
Player p;
PImage grass, ground,crate,red_brick, brown_brick, woman, british, player,men;
ArrayList<Sprite> platforms;
ArrayList<Sprite> women;
...
void setup(){
  ...
  woman=loadImage("data/women1.png");
  british=loadImage("data/british_right.png");
  men=loadImage("data/men_right.png");
  gameloose_music = new SoundFile(this, "data/gameloose.mp3");
  attack_music = new SoundFile(this, "data/jump2.wav");
  women_music=new SoundFile(this,"data/coin.mp3");
  //jump_music=new SoundFile(this,"data/jump_horse.mp3");
  women=new ArrayList<Sprite>();
...
void displayAll(){
  p.display();
   ...
   fill(255,255,255);
   textSize(32);
   text("    Durga Dal : "+numWomen,50*1.5+view_x,50*1.5+view_y);
   textSize(32);
   ...
   if(isGameOver){
     ...
     text("You lose!",view_x+width/2-100,view_y+height/2+50);
      text("You have lost all your lives!",view_x+width/2-100,view_y+height/2+100);
   }
     else{
     text("You win!",view_x+width/2-100,view_y+height/2+50);
     text("You have collected all Durga Dal!",view_x+width/2-100,view_y+height/2+100);}
...
void checkDeath(){
  ...
  if(collideEnemy ||collideMen|| fallOffCliff){
    gameloose_music.play();
    p.lives--;
    if(p.lives==0){
      gameloose_music.play();
    //delay(1000);
    //gameloose_music.stop();
    isGameOver=true;}
    else{
    p.center_x=100*1.5;
    p.setBottom(GROUND_LEVEL);}
...
void collectWomen(){
   ArrayList<Sprite> col_list = checkCollisionList(p, women);
   if(col_list.size() > 0){
    for(Sprite woman: col_list){
      women_music.play();
       women.remove(woman);
       numWomen++;
    }
  ...
  for(int row=0;row<lines.length;row++){
    ...
    for(int col=0; col<values.length;col++){
      if(values[col].equals("1")){
        ...
        Women c=new Women(woman,SPRITE_SCALE);
        ...
        GenderBiased gender=new GenderBiased(men,SPRITE_SCALE,bLeft,bRight);
